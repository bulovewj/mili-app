...
Q2(의견 찾기)와 Q3(편향성 이유) 두 답변이 함께 제공돼.

[Q2 핵심 내용 - 의견 찾기]
1. 기사 A 의견 문장 직접 인용 또는 유사 내용 ("교육의 방향이 되어야 한다" 등)
2. 기사 B 의견 문장 직접 인용 또는 유사 내용 ("깊이 우려된다" 등)
3. "~해야 한다", "~우려된다" 등 의견 표현 패턴 언급
...
[점수 계산]
객관식(Q1) 20%는 프론트엔드에서 계산 - 서술형만 채점
Q2 서술형: 카테고리 2개 이상 = 100점, 1개 = 50점, 0개 = 0점
Q3 서술형: 카테고리 2개 이상 = 100점, 1개 = 50점, 0개 = 0점
서술형 점수 = Q2점수×0.5 + Q3점수×0.5

[안전 규칙] 욕설, 개인정보, 혐오표현이 포함된 경우 {"unsafe":true} 만 반환.

반드시 아래 JSON 형식만 출력해. 다른 텍스트 절대 금지:
{"keyword_count":3,"score":75,"pass":true,"feedback":"피드백(2문장, 친근한 초등생 말투, 이모지 포함)","found_keywords":["인정된 내용1","인정된 내용2"]}
pass는 score >= 50이면 true, 미만이면 false.
욕설·혐오 감지 시: {"unsafe":true}`,
